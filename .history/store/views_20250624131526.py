...
def driver_dashboard(request):
    ...
    total_earnings = Order.objects.filter(assigned_to=driver, status='Delivered').aggregate(total=Sum('total_price'))['total'] or 0
    
    # Add location info for each order
    for order in orders:
        customer_profile = order.customer.customerprofile  # Assuming a related field `profile` in `CustomerProfile`
        if customer_profile:
