...
from django.contrib import messages
from django.shortcuts import render, redirect
from .models import DeliveryDriver, Order  # Update with the correct model
from django.contrib.auth.decorators import login_required
...
def driver_dashboard(request):
    ...
    # Get recent orders assigned to the logged-in driver (assuming the driver is assigned to the orders)
    recent_orders = Order.objects.filter(assigned_to=driver).order_by('-created_at')[:5]  # Fetch the last 5 orders
    total_deliveries = Order.objects.filter(assigned_to=driver, status='Delivered').count()
    total_earnings = Order.objects.filter(assigned_to=driver, status='Delivered').aggregate(total=models.Sum('total_price'))['total'] or 0
