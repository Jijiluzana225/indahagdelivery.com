...
from django.shortcuts import render, get_object_or_404
from .models import Order, OrderItem, Store  # Ensure Store is imported

def order_items(request, order_id):
    order = get_object_or_404(Order, id=order_id)
    items = OrderItem.objects.filter(order=order)

    # Retrieve the store linked to the order (assuming there's a relationship)
    store = order.store  # Adjust this based on your actual model structure

    context = {
        'order': order,
        'items': items,
        'store': store,  # Pass store object to the template
    }
    return render(request, 'store/order_items.html', context)
