...
def order_items_driver(request, order_id):
    order = get_object_or_404(Order, id=order_id)
    ...
    return render(request, 'store/order_items_driver.html', {'order': order, 'items': items, 'item.total_price':item.total_price})
...
def update_order_status_undelivered(request, order_id):
    order = get_object_or_404(Order, id=order_id)
    
    if order.status != 'Undelivered':  # Only allow status change if it's not already "Delivered"
...
def update_order_status_cancelled(request, order_id):
    order = get_object_or_404(Order, id=order_id)
...
@login_required
def order_items_view_driver(request, order_id):
    order = get_object_or_404(Order, id=order_id)
    ...
    return render(request, 'store/order_items_update_driver.html', {
        'order': order,
        'items': items,
        'products': products,
        'form': form
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Optionally log the exception details for further inspection
                print(f"Error occurred: {e}")  # This will log the error to your console or server logs
        else:
...
from django.contrib.auth import authenticate, login
from django.contrib import messages
from .forms import DeliveryDriverLoginForm
