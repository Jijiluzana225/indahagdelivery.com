...
class DriverDashboardView(LoginRequiredMixin, View):
    """Main dashboard view for delivery drivers"""
    ...
    def get(self, request):
        try:
            ...
            # Redirect to driver registration if profile doesn't exist
            return redirect('driver_registration')

@method_decorator(csrf_exempt, name='dispatch')
class UpdateDriverAvailabilityView(LoginRequiredMixin, View):
    """API endpoint to update driver availability status"""
    
    def post(self, request):
        try:
            # Get the driver instance
            driver = get_object_or_404(DeliveryDriver, user=request.user)
...
def driver_stats_api(request):
    """API endpoint to get driver statistics"""
    try:
        driver = get_object_or_404(DeliveryDriver, user=request.user)
        
        # You can add more complex stats calculation here
        # For example, weekly earnings, recent orders, etc.
        ...
        stats = {
            ...
            'weekly_deliveries': 12,  # Calculate from your Order model
            'weekly_earnings': 1850.00,  # Calculate from your Order model
            'pending_orders': 3,  # Calculate from your Order model
        }
...
# Alternative function-based view for availability update
@login_required
@require_http_methods(["POST"])
def update_driver_availability(request):
    """Function-based view alternative for updating driver availability"""
    try:
        driver = get_object_or_404(DeliveryDriver, user=request.user)
        ...
        return JsonResponse({
            'success': True,
            'is_available': driver.is_available,
            'message': f'Driver is now {"available" if driver.is_available else "unavailable"}'
        })
